from preprocess.frequencies import termsFrequency
from preprocess.compute_memberships import membershipMatrix


def exclude_rare(min_freq, cids, smiles, names, formulas, terms, tset):
    cids_out, smiles_out, names_out, formulas_out, terms_out, tset_out = [], [], [], [], [], []

    # Compute the terms memberships matrix (in order to compute frequencies of the terms)
    m, term_labels = membershipMatrix(tset, terms)
    frequency = termsFrequency(m)
    t2freq = dict()
    for fr, te in zip(frequency, term_labels):
        t2freq[te] = fr

    for c, s, n, f, t_list in zip(cids, smiles, names, formulas, terms):
        t_list_out = []
        for t in t_list:
            if t2freq[t] >= min_freq:
                t_list_out.append(t)
        if len(t_list_out) > 0:
            cids_out.append(c)
            smiles_out.append(s)
            names_out.append(n)
            formulas_out.append(f)
            terms_out.append(t_list_out)

        for t_list in terms_out:
            for t in t_list:
                tset_out.append(t)

        tset_out = list(set(tset_out))

    return cids_out, smiles_out, names_out, formulas_out, terms_out, tset_out


def exclude_duplicate(cids, smiles, names, formulas, terms):
    '''Found a set of SMILES, it will save only the first element with each SMILES'''
    smiles_set = set(smiles)
    cids_out, smiles_out, names_out, formulas_out, terms_out, tset_out = [], [], [], [], [], []

    for sm in smiles_set:
        ind = smiles.index(sm)
        cids_out.append(cids[ind])
        smiles_out.append(smiles[ind])
        names_out.append(names[ind])
        formulas_out.append(formulas[ind])
        terms_out.append(terms[ind])

    for t_list in terms_out:
        for t in t_list:
            tset_out.append(t)

    tset_out = list(set(tset_out))

    return cids_out, smiles_out, names_out, formulas_out, terms_out, tset_out


def exclude_size(cids, smiles, names, formulas, terms, tset, min_size, max_size):
    cids_out, smiles_out, names_out, formulas_out, terms_out, tset_out = [], [], [], [], [], []
    
    for c, s, n, f, t_list in zip(cids, smiles, names, formulas, terms):
        if min_size <= len(s) <= max_size:
            cids_out.append(c)
            smiles_out.append(s)
            names_out.append(n)
            formulas_out.append(f)
            terms_out.append(t_list)

    for t_list in terms_out:
        for t in t_list:
            tset_out.append(t)

    tset_out = list(set(tset_out))
            
    return cids_out, smiles_out, names_out, formulas_out, terms_out, tset_out
